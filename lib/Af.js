...
class Af extends EventEmitter {
    ...
    * @param {Qos|null} [qos]
     ...
     * @param {any} targetEp
     * @param {any} remoteEp
     * @param {'dataConfirm'|'reflectError'|'incomingMsg'|'incomingMsgExt'} type
     * @param {any} msg
    * @returns {any}
     */
    dispatchIncomingMsg(targetEp, remoteEp, type, msg) {
        ...
        try {
            ...
            debug(`Error parsing ZCL ${zclHeader && zclHeader.frameCntl && zclHeader.frameCntl.frameType ? 'functional' : 'foundation'} packet: ${ex}`);
            return
        }
     ...
     * @param {number|string} dstAddrOrGrpId
     * @param {number} cId
     * @param {Buffer} rawPayload
     * @param {import('./types').AfExtOptions} [opt]
    * @returns {any}
     */
    ...
    /**
     * @param {{addOnCancel: (fn: () => void) => void}} cancellationState
     * @param {any} srcEp
     * @param {any} dstEp
    * @param {any} cId
     * @param {Buffer} rawPayload
     * @param {import('./types').AfSendOptions} [opt]
     * @returns {Promise<any>}
     */
    async _send(cancellationState, srcEp, dstEp, cId, rawPayload, opt = {}) {
        ...
        function areqC(){
            ...
            ret.cancel = function(){
                const cancels = this._cancels || new Set()
    ...
    /**
     * @param {Promise<unknown[]> & { cancel: () => void }} areq
     ...
     * @param {Record<string, unknown>|unknown[]} zclData
     * @param {import('./types').ZclRequestConfig} cfg
     * @returns {Promise<any>}
     */
    async _zclFoundation(srcEp, dstEp, cId, cmd, zclData, cfg) {
        ...
        if (frameCntl.direction === 0 && !cfg.response) {    // client-to-server, thus require getting the feedback response
            ...
            areq = this.waitFor(mandatoryEvent, {filter:arg0=>((/** @type {any} */ (arg0)).zclData.cmdId === 'defaultRsp' || (/** @type {any} */ (arg0)).zclData.cmdId === foundationCommand.rsp)})
        }
    ...
    /**
     * @param {any} dstEp
     * @param {true|Record<string, unknown>|undefined} interested
    * @returns {any}
     */
     ...
     * @param {any} srcEp
     * @param {any} dstEp
     * @param {EventEmitter|undefined} eventEmitter
     * @param {true|Record<string, unknown>|undefined} interested
    * @returns {Promise<any>}
     */
     ...
     * @param {any} dstEp
     * @param {number|string} cId
     * @param {boolean|undefined} interestedValue
    * @returns {Promise<any>}
     */
    ...
    * @returns {Promise<any>}
    ...
    /**
     * @param {number|string} cId
     * @param {Array<{attrId: number, status: number, attrData: unknown}>} attributes
    * @returns {any}
     */
    ...
    * @returns {Promise<any>}
    ...
    async zclClusterAttrIdsReq(srcEp, dstEp, cId) {
        ...
        const attrsToRead = [];
        let startAttrId = /** @type {number|false} */ (0)
        do {
    ...
    * @returns {any}
    ...
    /**
     * @param {number[]} srcRtg
     * @returns {{relaycount: number, relaylist: number[]}}
     */
