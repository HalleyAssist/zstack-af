...
class Af extends Emittery {
    ...
    emit(eventName, ...args) {
        // Emittery's emit is async and takes a single data argument
        // We need to adapt the old API that passed multiple args
        // We don't await the emit calls to maintain the fire-and-forget behavior
        const eventData = args.length === 1 ? args[0] : args
        super.emit(eventName, eventData).catch(() => {})
        super.emit("all", { eventName, args: eventData }).catch(() => {})
    }

    // Bridge method to provide waitFor functionality similar to eventemitter2
    // Returns a cancelable promise that resolves when event is emitted
    waitFor(eventName, optionsOrTimeout) {
        let timeout = 0
        let filter = null
        
        if (typeof optionsOrTimeout === 'number') {
            timeout = optionsOrTimeout
        } else if (typeof optionsOrTimeout === 'object' && optionsOrTimeout !== null) {
            timeout = optionsOrTimeout.timeout || 0
            filter = optionsOrTimeout.filter || null
        }
        
        // If there's a filter, we need to handle it manually since emittery 0.13.1 doesn't support it
        let oncePromise
        let unsubscribe
        
        if (filter) {
            // Manual filter implementation
            oncePromise = new Promise((resolve, reject) => {
                ...
                unsubscribe = super.on(eventName, listener)
            })
            
            // Create an off method for the manual filter implementation
            oncePromise.off = () => {
                if (unsubscribe) unsubscribe()
            }
        } else {
            // Use native once which returns a promise with an off method
            oncePromise = super.once(eventName)
        }
        
        // Wrap the result in an array to match eventemitter2's waitFor behavior
        // eventemitter2's waitFor returns Promise<Array> of all arguments passed to emit
        const promise = oncePromise.then(eventData => {
            // Our emit wraps args, so we need to unwrap if it was a single arg
            // or keep as array if it was multiple args
        ...
        // Add cancel and off methods for compatibility with eventemitter2's waitFor
        const offMethod = oncePromise.off ? oncePromise.off.bind(oncePromise) : () => {}
        promise.cancel = offMethod
        promise.off = offMethod
        
        // If timeout is specified, race the promise with a timeout
        if (timeout > 0) {
            ...
            racedPromise.cancel = promise.cancel
            racedPromise.off = promise.off
            return racedPromise
        }
    ...
    nextZclSeqNum() {
        this._seq ++; // seqNumber is a private let on the top of this module
    ...
    async _send(cancellationState, srcEp, dstEp, cId, rawPayload, opt = {}) {
        ...
        let search = 0
        while (this.listenerCount(afEventCnf) > 0 && search++ < this.maxTransactions) {
    ...
    async _zclFoundation(srcEp, dstEp, cId, cmd, zclData, cfg) {
        ...
        let afOptions = cfg.afOptions !== undefined ? cfg.afOptions : {}
    ...
    async zclClustersReq(srcEp, dstEp, eventEmitter, interested) {    // callback(err, clusters)
        ...
        for (const cId in clusters) {
            ...
            i++;
            if (eventEmitter instanceof Emittery && !error) {
