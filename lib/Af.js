    ...
    dispatchIncomingMsg(targetEp, remoteEp, type, msg) {
            ...
            case 'zclIncomingMsg': {
                ...
                if (remoteEp) {
                    if (msg.zclMsg.cmdId === 'statusChangeNotification' && frameType === 1 && msg.zclMsg.payload) {
                        this.emit('ind:statusChange', { ep: remoteEp, cId: msg.clusterid, payload: msg.zclMsg.payload, msg });
                    }

                    if (frameType === 0 && msg.zclMsg.cmdId === 'report')
                        this.emit('ind:reported', { ep: remoteEp, cId: msg.clusterid, attrs: msg.zclMsg.payload });
                }
        ...
        if (zclHeader) {  // if (zclHeader && targetEp.isZclSupported()) {
            ...
            try {
                ...
                debug(`Error parsing ZCL ${zclHeader.frameCntl.frameType ? 'functional' : 'foundation'} packet: ${ex}`);
            }
    ...
    async zclReadAllAttributes(srcEp, dstEp, cId, attrIds) {
        ...
        const handleRequest = async () => {
            ...
            try {
                ...
                if (err.code == "ETIMEDOUT") {
                    debug("A timeout occured when reading attributes from cluster: %s. Error: %s", cId, err);
                } else {
...
Af.msgHandlers = [
    { evt: 'AF:dataConfirm', hdlr: 'dataConfirm', self: false },
    { evt: 'AF:reflectError', hdlr: 'reflectError', self: false },
    { evt: 'AF:incomingMsg', hdlr: 'incomingMsg', self: false },
    { evt: 'AF:incomingMsgExt', hdlr: 'incomingMsgExt', self: false },
    { evt: 'ZCL:incomingMsg', hdlr: 'zclIncomingMsg', self: true }
];
