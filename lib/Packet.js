...
let DChunks = require('dissolve-chunks'),
    ru = DChunks.Rule;

let FoundPayload = require('./packet/Foundation'),
...
zcl.parse = function (zclBuf, clusterId) {
    ...
    let zclObj = zclPayloadObject(data.frameCntl, clusterId, data.cmdId);
...
/**
 * @param {any} frameCntl
 * @param {number} manufCode
 * @param {number} seqNum
 * @param {any} cmd
 * @param {any} zclPayload
 * @param {any} [clusterId]
 * @returns {Buffer}
 */
...
ru.clause('zclFrame', function (bufLen) {
    ...
    this.uint8('frameCntl').tap(function () {
        let filedValue = this.vars.frameCntl;
