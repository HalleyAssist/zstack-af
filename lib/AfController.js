...
class AfController extends Emittery {
    constructor(){
        super()
        // Note: Emittery doesn't have setMaxListeners, but it also doesn't have the same memory leak warnings
        this._trans = 0
    }

    eventListenerCount(){
        // Emittery doesn't have eventNames(), so we can't implement this the same way
        // This method is not critical for operation, so we return an empty object
        // If needed, we could maintain our own event registry
        return {}
    }
