...
class Af extends EventEmitter {
    ...
    async send (srcEp, dstEp, cId, rawPayload, opt = {}) {
        ...
        try {
            const ret = await controller.indirectSend(deferred.promise, afSend, {signalTimeout, indirectTimeout:this.indirectTimeout, dstAddr:dstEp.getNwkAddr(), retries:opt.retries})
            return ret
        } catch(ex){
