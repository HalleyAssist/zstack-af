...
class Af extends EventEmitter {
    ...
    async send (srcEp, dstEp, cId, rawPayload, opt = {}) {
        ...
        const afSend = async () => {
            ...
            if(!isBroadcast && dstEp.getSrcRtg && !isResend){
                ...
                if(srcRtg){
                    const newAfParams = Object.assign(Af.buildAfSrcRtg(srcRtg), afParams, {afOptions: afParams.options & ~ZSC.AF.options.DISC_ROUTE})
    ...
    */
    ...
    makeAfParams(srcEp, dstEp, cId, rawPayload, opt) {
        ...
        let afOptions = ZSC.AF.options.DISC_ROUTE    // ACK_REQUEST (0x10), DISC_ROUTE (0x20)
