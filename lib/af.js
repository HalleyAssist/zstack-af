...
class Af extends EventEmitter {
    ...
    // }
        ...
        for(const c in clusters){
            let attrs, error
            const valueInterested = (interested === true || typeof interested === "undefined" || interested[c])
            try {
            ...
            Object.assign(clusters[c], {i, attrs, error})
        }
    ...
    async _zclClusterAttrsReq (dstEp, cId, interestedValue, limit) {
        ...
        if(interestedValue === false){
            ...
            await this.zclReadAllAttributes(dstEp, cId, attrIds, limit)
        }
        
        return this._mapAttributes(attributes);
    }
