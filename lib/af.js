...
class Af extends EventEmitter {
    ...
    dispatchIncomingMsg(targetEp, remoteEp, type, msg) {
        ...
        switch (type) {
            case 'dataConfirm':
                ...
                this.emit('AF:dataConfirm:' + msg.endpoint + ':' + msg.transid, msg);  // sender(srcEp) is listening, see send() and sendExt()
    ...
    async send (srcEp, dstEp, cId, rawPayload, opt = {}) {
        ...
        indirecter = controller._indirect_send(dstEp.getNwkAddr(), async function _sendBasic(){
            for(let i = 1; i<=retries; i++){
                try {
                    const rsp = await controller.request('AF', 'dataRequest', afParams)
                    
                    areq.setTimeout(afEventCnf, areqTimeout)
                    if (!apsAck)
                        areq.resolve(afEventCnf, rsp);

                    return rsp // don't loop
                } catch(err){
                    if(err.code){
                        if(err.code == 0x10 || err.code == 0x11 || err.code == 0xb2){
                            lastError = err
                            if(i != retries) await Q.delay(3000)
                            continue
                        }

                        areq.reject(afEventCnf, new Error('AF:dataRequest failed, status code: ' + err.code + '.'));
                        return
                    }
    ...
    async zclFunctional (srcEp, dstEp, cId, cmd, zclData, cfg) {
        ...
        assert (dstEp.isEndpoint && dstEp.isEndpoint(false), 'dstEp should be an instance of Endpoint class.')
