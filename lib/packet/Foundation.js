    ...
    frame (payload) { // args can be an array or a value-object if given
        ...
        switch (this.cmd) {
            ...
            case 'discoverRsp': {
                ...
                const discoverPayload = /** @type {any} */ (payload)
                dataBuf = dataBuf.uint8(discoverPayload.discComplete);
                for(const argObj of discoverPayload.attrInfos){
...
/*************************************************************************************************/
/**
 * @param {'multi'|'attrVal'|'attrValStruct'|'attrValStructNip'|'selector'} rule
 * @param {any} arg
 * @returns {any}
 */
...
ru.clause('selector', function () {
    ...
    this.tap('selector', function () {
        const scoped = /** @type {any} */ (this)
        scoped.uint8('indicator').tap(function () {
            const scopedTap = /** @type {any} */ (this)
            if (!scopedTap.vars.indicator) {
                scopedTap.indexes = [];
            } else {
                scopedTap.loop('indexes', function (end) {
                    parsedBufLen += 2;
                    this.uint16();

                    count += 1;
                    if (count === scopedTap.vars.indicator) end(); 
                }).tap(function () {
                    let tempArr = [];
                    scopedTap.vars.indexes.forEach(function (index) {
                        tempArr.push(index.undefined);
                    });
                    scopedTap.vars.indexes = tempArr;
                });
            }
        });
    }).tap(function () {
        const scopedTap = /** @type {any} */ (this)
        delete scopedTap.vars.selector.__proto__;
    });
});
