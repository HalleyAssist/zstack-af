...
class FuncPayload {
    ...
    constructor(clusterId, direction, cmd){
        ...
        this.direction = /** @type {any} */ (direction).key
        if (!this.direction)
    ...
    frame (args) { // args can be an array or a value-object if given
        ...
        if (params) {
            if(params.constructor.name !== 'Map'){
                ...
                const frameName = /** @type {any} */ (args._name)
                if(!params[frameName]) throw new Error("Unknown frame name")
                params = params[frameName]
            }
    ...
    args.forEach(function (arg, _idx) { // arg: { name, type, value }
        ...
        switch (type) {
            ...
            case 'longaddr':       // string '00124b00019c2ee9'
                dataBuf = dataBuf.uint32le(parseInt(val.slice(8), 16)).uint32le(parseInt(val.slice(0,8), 16));
                break;
            case 'stringPreLen':
